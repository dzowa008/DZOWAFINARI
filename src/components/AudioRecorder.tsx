...
// Enhanced waveform component with real-time visualization
const EnhancedWaveform = ({ 
  color = "from-fuchsia-600 to-indigo-400", 
  isRecording = false,
  audioData = null,
  isPlaying = false,
  currentTime = 0,
  ...
  const [bars, setBars] = useState(Array.from({ length: 64 }, () => Math.random() * 0.5 + 0.1));
  
  useEffect(() => {
    if (isRecording) {
      const interval = setInterval(() => {
        setBars(Array.from({ length: 64 }, () => Math.random() * 0.8 + 0.2));
      }, 100);
  ...
  return (
    <div className="flex gap-0.5 w-full h-12 items-end my-2 select-none relative">
      {bars.map((bar, i) => {
        const isActive = isPlaying && i < progress * bars.length;
        const height = isRecording ? bar : (isActive ? 0.8 : 0.3);
      ...
      {isPlaying && (
        <div 
          className="absolute top-0 bottom-0 w-1 bg-yellow-400 rounded-full transition-all duration-75"
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          className="absolute bottom-full right-0 mb-2 p-3 bg-slate-800 rounded-lg shadow-xl border border-slate-700 z-50 min-w-48"
        >
          <div className="text-sm font-semibold text-white mb-2">Audio Quality</div>
          {[
            { value: 'low', label: 'Low (32kbps)', desc: 'Smaller files, lower quality' },
            { value: 'medium', label: 'Medium (128kbps)', desc: 'Balanced quality & size' },
            { value: 'high', label: 'High (320kbps)', desc: 'Best quality, larger files' }
          ].map(option => (
            <label key={option.value} className="flex items-start gap-2 p-2 hover:bg-slate-700 rounded cursor-pointer">
              <input
                type="radio"
                name="quality"
                value={option.value}
...
export default function AudioRecorder({
  ...
  const [showAIEnhance, setShowAIEnhance] = useState<string | null>(null);
  ...
  useEffect(() => {
    if (!playingId) return;
    const interval = setInterval(() => setRerender({}), 100);
    return () => clearInterval(interval);
  }, [playingId]);

  // Sync playback rate and volume on all audio elements
  useEffect(() => {
  ...
  }, [playbackRate, volume, isMuted, playingId]);
  ...
  const handleAIEnhance = async (noteId: string) => {
    ...
    if (note && onEditAudioNote) {
      const enhancedNote = {
        ...note,
        content: `${note.content}\n\n🤖 **AI Enhanced Summary:**\nThis audio recording has been analyzed and enhanced with AI-powered insights. Key topics identified include important concepts, action items, and follow-up tasks.`,
        summary: `AI-enhanced analysis completed. Key insights extracted and organized for better understanding.`,
        tags: [...(note.tags || []), 'ai-enhanced', 'analyzed'],
        updatedAt: new Date()
      };
      onEditAudioNote(enhancedNote);
    }
  ...
  const getAudioInfo = (noteId: string) => {
    const audio = audioRefs.current[noteId];
    if (!audio) return { currentTime: 0, duration: 1, progress: 0 };
  ...
  return (
    <div className="max-w-6xl mx-auto py-16 px-5 space-y-12 min-h-screen relative">
      ...
      >
        ...
        <div className="absolute left-10 top-8">
          <AudioQualitySettings
            ...
            onToggle={() => setShowQualitySettings(!showQualitySettings)}
          />
        </div>
        ...
        <div className="flex flex-col items-center mb-2">
          ...
          <h2 className="text-2xl font-bold text-white tracking-tight">Enhanced Audio Recorder</h2>
          <div className="mt-1 text-slate-300 text-base">AI-Powered • High Quality • Smart Analysis</div>
        </div>
        ...
        {isRecording && (
          <div className="w-full flex flex-col items-center pt-2">
            ...
            {transcriptionSupported && (
              <div className="bg-slate-900/90 p-4 rounded-xl text-slate-100 w-full max-w-lg shadow-inner text-base border border-slate-700/50">
                <div className="flex items-center gap-2 mb-2">
                  <Brain className="w-4 h-4 text-fuchsia-400" />
                  <span className="text-fuchsia-300 font-semibold">Live AI Transcription</span>
                </div>
                
                {finalTranscription && (
                  <div className="mb-2 p-2 bg-green-900/30 rounded border border-green-700/30">
                    <div className="text-green-400 font-bold text-sm mb-1">✓ Final:</div>
                    <div className="text-white">{finalTranscription}</div>
                  </div>
                )}
                
                {currentTranscription && (
                  <div className="p-2 bg-yellow-900/30 rounded border border-yellow-700/30">
                    <div className="text-yellow-400 font-bold text-sm mb-1">🎤 Live:</div>
                    <div className="text-white italic">{currentTranscription}</div>
                  </div>
                )}
      ...
      <section className={`${glass} rounded-3xl p-8`}>
        <div className="text-xl text-white font-bold mb-5 select-none flex items-center gap-2">
          {viewMode === 'recordings' ? (
            <>
        ...
        {viewMode === 'recordings' ? (
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-9">
            ...
            {filteredNotes.map(note => {
              ...
              const { currentTime, duration, progress } = getAudioInfo(note.id);
              ...
              return (
                <motion.div
                  ...
                  viewport={{ once: true }}
                  className="bg-slate-950/80 border border-fuchsia-900/20 rounded-2xl shadow-lg p-5 flex flex-col gap-2 group relative min-h-[280px] hover:scale-105 transition"
                  tabIndex={-1}
                ...
                >
                  ...
                  {note.tags?.includes('ai-enhanced') && (
                    <div className="absolute top-3 left-3 bg-gradient-to-r from-purple-500 to-pink-500 text-white font-bold text-xs rounded-full px-2 py-0.5 select-none">
                  ...
                  <div className="flex items-center gap-2 text-xs text-slate-400 mb-2">
                    ...
                    <span>•</span>
                    <span className="capitalize">{audioQuality} quality</span>
                  </div>

                  {/* Enhanced waveform + playback controls */}
                  ...
                  {/* Enhanced transcription display */}
                  <div className="bg-slate-900/60 rounded-lg p-3 font-mono text-slate-100 text-sm mt-2 mb-1">
                    <div className="flex items-start justify-between mb-1">
                      <span className="text-fuchsia-300 text-xs font-semibold">Transcription</span>
                      {!!note.transcription && (
                        <button
                    ...
                    <div className="text-xs leading-relaxed max-h-20 overflow-y-auto">
                      {note.transcription
                        ? note.transcription.length > 150 ? note.transcription.slice(0, 150) + '…' : note.transcription
                        : "Transcribing…"
                      }
                    </div>
                  </div>
                  ...
                  {note.tags?.length > 0 && (
                    <div className="mb-1 flex flex-wrap gap-1 mt-1" aria-label="Tags">
                      {note.tags.map((tag, i) => (
                        <span
                          key={i}
                          className={`inline-block rounded-full px-2 py-0.5 text-xs font-semibold select-none ${
                            tag === 'ai-enhanced' 
                  ...
                  {note.summary && (
                    <div
                      className="mt-1 bg-slate-900/90 rounded-lg p-2 text-fuchsia-200 text-xs font-mono cursor-pointer hover:bg-indigo-900/80 select-text"
                  ...
                  <div className="flex gap-2 mt-auto">
                    ...
                    <button
                      onClick={() => handleAIEnhance(note.id)}
                      disabled={isEnhancing === note.id || note.tags?.includes('ai-enhanced')}
                      className="flex-1 px-2 py-1 bg-purple-600/80 hover:bg-purple-800 disabled:bg-slate-700 text-white rounded text-xs flex items-center gap-1 justify-center disabled:cursor-not-allowed"
                      aria-label={`Enhance with AI: ${note.title}`}
                      type="button"
                      title={note.tags?.includes('ai-enhanced') ? 'Already AI enhanced' : 'Enhance with AI analysis'}
                    >
                      {isEnhancing === note.id ? (
                      ...
                      {isEnhancing === note.id ? 'Enhancing...' : 'AI Enhance'}
                    </button>
                  ...
                  <div className="flex gap-2 mt-2">
                    ...
                    <button
                      onClick={() => navigator.clipboard.writeText(note.audioUrl || '')}
                      className="flex-1 px-2 py-1 bg-slate-700/80 hover:bg-slate-900 text-white rounded text-xs flex items-center gap-1 justify-center"
                      aria-label={`Copy audio URL: ${note.title}`}
          ...
          <div className="max-w-4xl mx-auto space-y-6 text-sm text-slate-300">
            {filteredNotes.length === 0 ? (
              ...
              filteredNotes.map(note => (
                <div key={note.id} className="bg-slate-900/70 p-5 rounded-lg shadow-md select-text border border-slate-700/50">
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        onClick={() => navigator.clipboard.writeText(note.transcription!)}
                        className="text-fuchsia-400 hover:text-fuchsia-600 transition text-xs flex items-center gap-1"
